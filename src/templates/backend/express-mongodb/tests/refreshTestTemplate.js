...
test('refreshes access token successfully', async () => {
    const refreshCookie = await signupAndSignin('shipsprint.refresh', 'shipsprint.refresh@gmail.com')
...
test('rejects refresh when refresh token is invalid', async () => {
    ...
    const response = await api
        .post('/api/auth/refresh')
        .set('Cookie', ['refreshToken=invalid-refresh-token', csrfCookie])
        .set('X-CSRF-Token', csrfToken)
...
test('rejects refresh when refresh token is expired', async () => {
    const refreshCookie = await signupAndSignin('shipsprint.expired', 'shipsprint.expired@gmail.com')
    const refreshTokenCookie = refreshCookie.find(cookie => cookie.startsWith('refreshToken='))
    ...
    const refreshTokenRecord = await refreshTokenModel.findOne({ tokenHash })
...
test('rejects refresh when refresh token is revoked', async () => {
    const refreshCookie = await signupAndSignin('shipsprint.revoked', 'shipsprint.revoked@gmail.com')
    const refreshTokenCookie = refreshCookie.find(cookie => cookie.startsWith('refreshToken='))
...
test('rejects refresh when old refresh token is reused', async () => {
    const originalRefreshCookie = await signupAndSignin('shipsprint.reuse', 'shipsprint.reuse@gmail.com')
    const { csrfCookie, csrfToken } = await getCsrfToken()
...
export default refreshTestTemplate;
