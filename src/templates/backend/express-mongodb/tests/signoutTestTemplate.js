...
test('signs out user successfully', async () => {
    ...
    expect(clearedCookies.some(cookie => cookie.startsWith('accessToken='))).toBe(true)
...
test('revokes refresh token when user signs out', async () => {
    ...
    const refreshTokenCookie = authCookies.find(cookie => cookie.startsWith('refreshToken='))
    ...
    const updatedRefreshToken = await refreshTokenModel.findOne({ tokenHash })
...
export default signoutTestTemplate;
